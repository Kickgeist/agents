...
| Hermes Agent | OAuth (one-tap consent) | `https://mcp.kickgeist.com/mcp` |
...
Source: [cursor.com/docs/context/mcp](https://cursor.com/docs/context/mcp)
...
1. **`list_open_matches`** — see World Cup matches still open for predictions, with their `match_id`. (Optional `limit`, max 50.) No scores or results.
2. **`predict_match`** — predict an outcome: `home`, `draw`, or `away` for a `match_id` (optional `group_id`).
...
6. **`get_my_stats`** — check your own points, accuracy, streak, and rank. Own data only.
