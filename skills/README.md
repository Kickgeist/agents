...
> **One account, two front rows.** Anything your agent does through these skills shows up in the [KICKGEIST mobile app](https://kickgeist.com), and vice-versa — as long as you save your recovery code. Every skill here treats that code as sacred.
...
- **Auth:** authless — add the URL, then your agent calls `create_account` to get an identity plus a recovery code.
...
Each skill lives in its own folder and is the **same playbook, packaged for the way your client loads instructions.** They all cover the full arc — get an account, find open matches, lock in predictions, start or join a group, check your stats — in one shared voice with the same guardrails. Pick the one that matches your agent.

| Skill | Use it when your agent is… | File |
|-------|-----------------------------|------|
| **Claude skill** | Claude or Claude Code, where you can drop in a skill folder. Front-matter-driven, auto-invoked when someone wants to predict a match, run a group, or check their stats. | [`claude-skill/SKILL.md`](./claude-skill/SKILL.md) |
...
Every skill here moves through the same nine tools in the same natural order. If you're writing your own skill, follow this spine:

1. **Get an identity first.** `create_account` for a fresh anonymous account (returns a **recovery code** — surface and save it immediately), or `link_account` with a `recovery_code` to connect to the account you already play on your phone. Use `get_recovery_code` any time to show the code for the linked account.
...
4. **Bring the friends.** `create_group` returns a shareable invite link (`https://kickgeist.com/join/{inviteCode}`); `join_group` takes a raw 6-character code **or** a full join link. `get_my_groups` lists what you're in, with invite links and your role.
...
- **The recovery code is the account.** There's no email or password behind it. Whenever an account is created or linked, surface the code clearly and tell the player to save it — losing it with no linked app means the account can't be recovered. Never print it where it might leak into shared logs; treat it like a secret.
...
- **Confirm before changing things that matter.** Money never enters the picture (KICKGEIST is zero-money), but a thoughtful agent still confirms before changing an already-placed pick or before creating a second account when one is already linked.
