...
1. **"Show me the matches I can predict right now."** Cascade calls `list_open_matches` and lists matches open for predictions — World Cup fixtures plus tagged warm-up friendlies — each with a `matchId`, the two teams, kickoff time, and stage.
...
| --- | --- | --- |
| `list_open_matches` | `limit?` (max 50) | Lists matches currently open for predictions — World Cup fixtures plus tagged warm-up friendlies — each with its stage. No scores, results, or finished matches. |
...
**How predictions work.** Each pick is `home`, `draw`, or `away`. World Cup matches open for predictions **36 hours before kickoff**; warm-up friendlies are open any time before kickoff; all lock at kickoff, and Cascade can change a pick freely until then. In the group stage, `draw` is a normal level result. In a knockout match a tie after extra time goes to a penalty shootout, which KICKGEIST scores as a **`draw`** — so predicting `draw` backs penalties, and the shootout winner doesn't change the result.
...
- Windsurf MCP setup: https://docs.devin.ai/windsurf/plugins/cascade/mcp
